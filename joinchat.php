...
$string = file_get_contents("chat/".$_POST['sel']."");
...
if ($num_str=='0'){
  echo'нет юзеров';
  file_put_contents("chat/".$_POST['sel']."",'');
...
$ok=$_SESSION['ok'];
$er=$_SESSION['er'];
echo 'Успешно вступили'.$ok.'<br>Не удалось вступить:'.$er.'<br>чатов в файле '.$num_str.'<br><br>';
...
try{
	...
	$_SESSION['ok']++;
$MadelineProto->channels->joinChannel(['channel' => $id]);
...
file_put_contents("chat/".$_POST['sel']."",json_encode($task_array));
